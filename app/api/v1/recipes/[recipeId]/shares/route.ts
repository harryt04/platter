...
import {
  ...
  ownedRecipeFilter,
  recipeShareFilter,
  recipeShareMutationResponseSchema,
  recipeSharingResponseSchema,
...
function sharingUnavailable() {
  return problemResponse({
    type: 'https://platter.dev/problems/recipe-sharing-unavailable',
    title: 'Recipe sharing temporarily unavailable',
    status: 503,
    detail: 'Recipe sharing could not be loaded. Try again shortly.',
    code: 'RECIPE_SHARING_UNAVAILABLE',
  })
}
...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    const response = recipeSharingResponseSchema.safeParse({
    ...
    if (!response.success) return sharingUnavailable()
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const response = recipeShareMutationResponseSchema.safeParse({
      visibility,
